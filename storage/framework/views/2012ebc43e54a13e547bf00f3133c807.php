...
<div class="space-y-6">
    ...
    <div class="flex justify-end mb-2">
        <?php if(function_exists('canSeeMenu') && canSeeMenu('usuarios')): ?>
                            ...
                            <div class="flex justify-end">
                                <div class="relative acciones-dropdown-container">
                                    ...
                                    <div class="absolute right-0 mt-2 w-48 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg shadow-lg z-50 acciones-dropdown-menu" style="display: none;">
                                        ...
                                        <?php endif; ?>
                                        <?php if(auth()->user()->hasRole('AGENCIERO')): ?>
                                            <a href="<?php echo e(route('admin.users.edit', $user)); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition-colors">
                                                <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                                Editar
                                            </a>
                                            <form method="POST" action="<?php echo e(route('admin.users.destroy', $user)); ?>" onsubmit="return confirm('¿Estás seguro de eliminar este usuario?');">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('users.change_permissions')): ?>
                                            <a href="<?php echo e(route('admin.users.permissions.edit', $user)); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition-colors">
                                        ...
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('audit.view_logs')): ?>
                                            <a href="<?php echo e(route('admin.users.activity', $user)); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] transition-colors">
                                        ...
                                        <?php endif; ?>
                                        <?php if(isset($availablePlans) && $availablePlans->count() > 0 && auth()->check() && in_array(auth()->user()->email, ['superadmin@autos.com', 'admin@autowebpro.com.ar']) && auth()->id() !== $user->id): ?>
                                            <button
                                                type="button"
                                                ...
                                                data-action="<?php echo e(route('admin.users.manual-enable-transfer', $user)); ?>"
                                            >
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <?php if(isset($availablePlans) && $availablePlans->count() > 0 && auth()->check() && in_array(auth()->user()->email, ['superadmin@autos.com', 'admin@autowebpro.com.ar'])): ?>
                    ...
                    <div>
                        ...
                        <select id="plan_slug" name="plan_slug" required
                            ...
                            <?php $__currentLoopData = $availablePlans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($plan->slug); ?>">
                                    <?php echo e($plan->nombre); ?> (<?php echo e($plan->slug); ?>) - $<?php echo e(number_format((float) $plan->price, 0)); ?> ARS
                                </option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
