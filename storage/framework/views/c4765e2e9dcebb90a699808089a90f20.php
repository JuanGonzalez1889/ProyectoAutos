...
function saveText(){
  ...
  fetch('<?php echo e(parse_url(route("admin.landing-config.update"), PHP_URL_PATH)); ?>',{method:'POST', headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json','X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content}, body:fd})
...
async function saveImage(){
  ...
  if(fileInput.files && fileInput.files[0]){
    ...
    try{
      const resp = await fetch('<?php echo e(parse_url(route("admin.landing-config.upload-image"), PHP_URL_PATH)); ?>',{method:'POST', headers:{'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content}, body:formData});
...
<?php endif; ?>
<?php /**PATH C:\Proyectos\ProyectoAutos\resources\views\public\templates\partials\editor-scripts.blade.php ENDPATH**/ ?>