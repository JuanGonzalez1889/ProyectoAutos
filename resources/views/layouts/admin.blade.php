                @php
                @endphp
                @if(canSeeMenu('planes por rol'))
                <div class="flex flex-col items-center justify-center py-6">
                    <a href="{{ route('admin.planes.configuracion') }}"
                        class="flex flex-col items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors {{ request()->routeIs('planes.configuracion') ? 'bg-[hsl(var(--primary))] text-white' : 'text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))] hover:text-[hsl(var(--foreground))]' }}">
...
<body class="bg-[hsl(var(--background))]">
    ...
    margin-top: 2rem;
">
                    <img src="{{ asset('storage/icono.png') }}" class="w-12 h-12 rounded" style="max-width:48px; max-height:48px; width:48px; height:48px;" alt="Logo">
...
">
                <div class="flex items-center gap-3 mb-3">
                    @if (auth()->user()->avatar)
                        <img src="{{ auth()->user()->avatar }}" class="sidebar-user-icon w-10 h-10 rounded-full"
                            alt="Avatar">
                    @else
                        <div
                            ...
                            <span class="text-white font-bold text-sm" style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                        </div>
                    @endif
                    <div class="flex-1 min-w-0 sidebar-user-info">
                        <p class="text-sm font-medium text-[hsl(var(--foreground))] truncate">{{ auth()->user()->name }}
                        </p>
                ...
                <div class="px-3 py-2 bg-transparent border border-[hsl(var(--border))] rounded-lg sidebar-role">
                    <span
                        class="text-xs text-[hsl(var(--foreground))]">{{ auth()->user()->roles->first()->name ?? 'Sin rol' }}</span>
                </div>
            </div>

            <!-- Navigation -->
            @php \Log::channel('single')->info('[sidebar-blade]', ['user_id' => auth()->user()->id ?? null, 'email' => auth()->user()->email ?? null]); @endphp
...
">
                ...
                <a href="{{ route('admin.events.calendar') }}"
                    ...
                    @if ($todayEventsCount > 0)
                        <span
                            class="px-1.5 py-0.5 text-[10px] font-medium rounded-full bg-orange-500/20 text-orange-500">{{ $todayEventsCount }}</span>
                    @endif
                </a>
                @endif

                @if(canSeeMenu('clientes'))
                    <a href="#"
                ...
                {{-- Ítem duplicado de Tareas eliminado. Solo queda "Mis Tareas" --}}
                @if(canSeeMenu('mi agencia'))
                ...
                @endif
                {{-- Plantillas oculto, solo se muestra Mi Web arriba --}}
                {{-- <a href="{{ route('admin.landing-config.show') }}"
                    ...
                    <span class="sidebar-label flex-1">Configurar Landing</span>
                </a> --}}
                @if(canSeeMenu('mis dominios'))
