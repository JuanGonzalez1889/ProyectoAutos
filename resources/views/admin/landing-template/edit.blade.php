        ...
        <div class="h-20 px-8 flex items-center justify-between">
            ...
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">{{ ucfirst($template) }} — Clickea los elementos para editar</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-6">
                <div>
                    ...
                    <div class="flex gap-2 items-center">
                        <input type="color" id="primaryColor" value="{{ $settings->primary_color ?? '#8b5cf6' }}" class="h-9 w-14 rounded cursor-pointer border border-slate-700">
                        <input type="text" id="primaryColorText" value="{{ $settings->primary_color ?? '#8b5cf6' }}" class="w-24 px-2 h-9 bg-slate-800 border border-slate-700 rounded text-xs font-mono text-white" readonly>
                    </div>
                </div>
                <div>
                    <label class="text-xs text-gray-400 block mb-2 font-medium">🎨 Color Secundario</label>
                    <div class="flex gap-2 items-center">
                        <input type="color" id="secondaryColor" value="{{ $settings->secondary_color ?? '#1e293b' }}" class="h-9 w-14 rounded cursor-pointer border border-slate-700">
                        <input type="text" id="secondaryColorText" value="{{ $settings->secondary_color ?? '#1e293b' }}" class="w-24 px-2 h-9 bg-slate-800 border border-slate-700 rounded text-xs font-mono text-white" readonly>
                    </div>
                </div>
                <div>
                    <label class="text-xs text-gray-400 block mb-2 font-medium">🎨 Color Terciario</label>
                    <div class="flex gap-2 items-center">
                        <input type="color" id="tertiaryColor" value="{{ $settings->tertiary_color ?? '#ffaa00' }}" class="h-9 w-14 rounded cursor-pointer border border-slate-700">
                        <input type="text" id="tertiaryColorText" value="{{ $settings->tertiary_color ?? '#ffaa00' }}" class="w-24 px-2 h-9 bg-slate-800 border border-slate-700 rounded text-xs font-mono text-white" readonly>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('public.landing') }}" target="_blank" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition flex items-center gap-2 shadow-lg">
    ...
    <script>
        ...
        window.saveColors = function() {
            ...
            const currentValues = {
                ...
                stat1: {!! json_encode($settings->stat1 ?? '') !!},
                stat2: {!! json_encode($settings->stat2 ?? '') !!},
                stat3: {!! json_encode($settings->stat3 ?? '') !!},
                stat1_label: {!! json_encode($settings->stat1_label ?? '') !!},
                stat2_label: {!! json_encode($settings->stat2_label ?? '') !!},
                stat3_label: {!! json_encode($settings->stat3_label ?? '') !!}
            };
