@extends('public.templates.clasico-base')
@section('content')
<style>
    .vehiculo-card-auto {
...
<div class="max-w-4xl mx-auto px-4 py-12" style="font-family: {{ $settings->font_family ?? 'inherit' }};">
    <div class="flex flex-col md:flex-row gap-8">
        <div class="md:w-1/2">
            <img id="main-vehicle-image" src="{{ $vehicle->main_image }}" alt="{{ $vehicle->title }}" class="w-full h-80 object-cover rounded-xl mb-4">
        ...
        <div class="md:w-1/2 flex flex-col">
            ...
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $vehicle->whatsapp ?? $settings->whatsapp ?? '') }}?text=Hola! Estoy interesado en el {{ $vehicle->title }}" target="_blank" class="w-full py-2 rounded font-semibold text-center block" style="background: var(--terciary-color); color: var(--primary-color);">Consultar por WhatsApp</a>
        </div>
    </div>
</div>
@endsection
