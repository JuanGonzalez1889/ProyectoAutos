...
<body style="background-color: var(--secondary-color);" class="text-white">
    ...
    <div class="max-w-6xl mx-auto px-6 py-0 border-b border-gray-700">
        <div class="flex flex-col md:flex-row items-stretch h-[350px] md:h-[400px] lg:h-[480px] mb-8">
            ...
            <div class="flex-1 flex flex-col justify-center items-start p-8 bg-transparent relative z-10">
                ...
                @else
                    <h1 class="text-5xl font-bold mb-4 auto-contrast-title">{{ $settings->hero_title }}</h1>
                @endif
                <div class="editable-section relative w-full">
                    <p class="text-xl max-w-2xl whitespace-pre-line" style="color: {{ $settings->home_description_color ?? '#fff' }}">
                ...
                @if(isset($editMode) && $editMode)
                    <div class="editable-section w-full h-full relative flex items-center justify-center">
                        @if($settings && $settings->banner_url)
                            <img src="{{ $settings->banner_url }}" alt="Banner" class="object-cover w-full h-full max-h-[400px] max-w-[400px] rounded shadow-lg border-0">
                        @else
        ...
        {{-- El include de editor-scripts.blade.php solo debe ir una vez al final --}}
    </div>
    ...
    <div id="nosotros" class="max-w-6xl mx-auto px-6 py-12 border-b border-gray-700">
        ...
        <div class="grid md:grid-cols-2 gap-8 items-start">
            <div class="space-y-4">
                <p class="text-lg leading-relaxed whitespace-pre-line" style="color: {{ $settings->nosotros_description_color ?? '#222' }}">{{ $settings->nosotros_description ?? 'Cuéntale a tus usuarios sobre tu agencia, experiencia y servicios.' }}</p>
