        ...
        <div class="flex flex-col gap-4 h-full">
            ...
            <div class="p-5 bg-gradient-to-br from-yellow-400/10 to-yellow-600/10 border border-yellow-500/20 rounded-lg h-full flex-1">
                <div class="flex items-start justify-between mb-2">
                    ...
                    <span class="text-[10px] px-1.5 py-0.5 bg-yellow-500/20 text-yellow-600 rounded">@if($stats['pending_leads'] > 0)+@endif{{ $stats['pending_leads'] }}</span>
                </div>
                <p class="text-xs text-[hsl(var(--muted-foreground))] mb-1">Leads Pendientes</p>
                <p class="text-2xl font-bold text-white">{{ $stats['pending_leads'] }}</p>
                {{-- <a href="{{ route('admin.leads.index') }}" class="block mt-2 text-xs text-yellow-700 underline hover:text-yellow-900">Ver leads</a> --}}
            </div>
            <!-- Unidades Vendidas -->
                    ...
                    <form id="monthRevenueForm" method="GET" action="" class="inline">
                        <select name="month" id="monthRevenueSelect" class="bg-green-900 text-white border border-green-400 rounded px-2 py-1 text-xs appearance-none" style="outline:none;background-color:#064e3b">
                            @for($i = 0; $i < 12; $i++)
                                ...
                                @endphp
                                <option value="{{ $value }}" {{ request('month', now()->format('Y-m')) == $value ? 'selected' : '' }} style="background-color:#064e3b;color:white;">{{ $label }}</option>
                            @endfor
                        </select>
                    </form>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="h-64 flex items-end justify-around gap-2">
                ...
                @forelse($monthValues as $value)
                    <div class="flex-1 bg-[hsl(var(--primary))]/50 rounded-t transition-all hover:bg-[hsl(var(--primary))]" 
                         style="height: {{ $maxValue > 0 ? max(10, ($value / $maxValue) * 90) : 10 }}%"
                         title="Ingresos: ${{ number_format($value, 0, ',', '.') }}">
                    </div>
                @empty
        ...
        <div class="space-y-6">
            ...
            <div class="card">
                ...
                <div class="space-y-3">
                    @forelse($stats['today_events'] ?? [] as $event)
                        <div class="flex gap-3">
                            ...
                            <div class="flex-1 pb-3">
                                ...
                                <p class="text-xs text-[hsl(var(--muted-foreground))]">{{ $event->description ?? 'Sin descripción' }}</p>
                            </div>
                        </div>
                    @empty
