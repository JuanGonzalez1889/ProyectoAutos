...
<div class="relative {{ $popular ? 'border-2 border-blue-500' : 'glass' }} rounded-xl p-8 hover-glow">
    @if($popular)
    ...
    <a href="{{ $ctaLink }}" class="block w-full py-3 {{ $popular ? 'btn-gradient' : 'bg-white/5 hover:bg-white/10' }} rounded-lg text-center font-semibold text-white transition-all">
        {{ $cta }}
    </a>
</div>
