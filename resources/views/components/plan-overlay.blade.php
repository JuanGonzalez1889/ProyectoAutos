...
@endphp
@if(auth()->user()->tenant && auth()->user()->tenant->getPlanInfo()['plan'] === 'free' && !$isOnPlansPage)
    <div id="plan-overlay" class="fixed inset-0 z-[9999] flex items-center justify-center bg-[hsl(var(--background)/0.7)] backdrop-blur-sm">
        <div class="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl shadow-2xl px-10 py-8 max-w-[90vw] text-center">
            ...
            <a href="{{ route('subscriptions.index') }}" class="inline-block px-8 py-3 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary)/.90)] text-white rounded-lg font-semibold text-lg mt-2 shadow transition">Seleccionar Plan</a>
            <br>
